...
### Subtasks
- [x] 1.1 Run a global replacement of `claude-starter-kit` → `claude-sak` across all files listed in the implementation plan (scripts, workflows, config, tests, fixtures, docs, commands)
- [x] 1.2 Verify workflow guards: confirm `template-sync.yml:28` and `template-cleanup.yml:43` now read `!= 'claude-sak'`
- [x] 1.3 Verify Serena config: confirm `.github/templates/serena/project.yml` `project_name` is `claude-sak`
