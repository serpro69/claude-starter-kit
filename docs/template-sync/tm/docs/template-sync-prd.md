...
Enable repositories created from the `claude-sak` template to receive configuration updates from the upstream template repository.

## Problem Statement

Currently, `claude-sak` functions as a one-time template:
...
As a developer using a repo created from claude-sak, I want to receive configuration updates when the template improves, so I can benefit from new skills, commands, and fixes.
