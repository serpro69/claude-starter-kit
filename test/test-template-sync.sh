...
result=$(resolve_version "v1.0.0" "serpro69/claude-sak")
...
reset_globals
set +e
result=$(resolve_version "master" "serpro69/claude-sak" 2>/dev/null)
exit_code=$?
set -e
...
reset_globals
set +e
result=$(resolve_version "HEAD" "serpro69/claude-sak" 2>/dev/null)
exit_code=$?
set -e
...
reset_globals
set +e
result=$(resolve_version "latest" "serpro69/claude-sak" 2>/dev/null)
exit_code=$?
set -e
...
result=$(resolve_version "v99.99.99" "serpro69/claude-sak")
...
assert_equals "serpro69/claude-sak" "$result" "upstream_repo rewritten to serpro69/claude-sak"
rm -rf "$test_dir"
