...
The old template-sync workflow and script contain taskmaster-specific sync logic that will cause issues on future syncs (see https://github.com/serpro69/claude-sak/issues/17).

If `.claude/scripts/sync-workflow.sh` does not exist, fetch it first:

```bash
mkdir -p .claude/scripts
curl -fsSL "https://raw.githubusercontent.com/serpro69/claude-sak/master/.github/templates/claude/scripts/sync-workflow.sh" \
