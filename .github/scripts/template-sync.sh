#!/usr/bin/env bash
#
# template-sync.sh - Synchronize configuration from upstream claude-sak template
#
...
#
#   "Version not found":
#     - Check available tags: git ls-remote --tags https://github.com/serpro69/claude-sak
...
# (serpro69/claude-starter-kit) to the new name (serpro69/claude-sak).
...
migrate_manifest() {
  ...
  if [[ "$upstream_repo" == "serpro69/claude-starter-kit" ]]; then
    ...
    if ! jq '.upstream_repo = "serpro69/claude-sak"' "$MANIFEST_PATH" > "$tmp"; then
      rm -f "$tmp"
...
Synchronizes template updates from the upstream claude-sak repository.
